...
$$
\begin{align*}
\inf_{π∈Π}{\cal{W}^{p}_{p}( \hat{ρ}_π,\hat{ρ}_e)}=\inf_{π∈Π}\inf_{θ∈Θ}∑_{i=1}^{T}∑_{j=1}^{D}{d((s^π_i,a^π_i),(s^e_j,a^e_j))^pθ[i,j]}
\end{align*}
$$
...
$$
\begin{align*}
\inf_{π∈Π}{\cal{W}^{p}_{p}(\hat{ρ}_π,\hat{ρ}_e)} &= \inf_{π∈Π}\sum^{T}_{i=1}{c^*_{i, π}} \\
