...
$$
\begin{align*}
\inf_{π∈Π}{\cal{W}^p_p(\hat{ρ}_π,\hat{ρ}_e)}=\inf_{π∈Π}\inf_{θ∈Θ}∑_{i=1}^{T}∑_{j=1}^{D}{d((s^π_i,a^π_i),(s^e_j,a^e_j))^pθ[i,j]}
\end{align*}
$$
...
$$
\begin{align*}
θ^∗_π = \argmin_{θ∈Θ}\sum^T_{i=1}\sum^D_{j=1}d((s^π_i, a^π_i),(s^e_j, a^e_j))θ[i, j]
\end{align*}
$$
...
$$
\begin{align*}
\inf_{π∈Π}{\cal{W}^p_p(\hat{ρ}_π,\hat{ρ}_e)} &= \inf_{π∈Π}\sum^{T}_{i=1}{c^*_{i, π}} \\
...
$$
\begin{align*}
θ^g_π[i, :] &= \argmin_{θ[i, :]∈Θ_i}\sum^D_{j=1}d((s^π_i, a^π_i),(s^e_j, a^e_j))θ[i, j] \\
...
<p align="center"> <img src="https://github.com/hongjun7/logs/blob/main/_posts/image/2024-02-16-PWIL/fig10.png?raw=true" width="100%"> </p>
